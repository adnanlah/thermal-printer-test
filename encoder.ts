...
type BufferValue = number | Buffer | Uint8Array

/**
 * Create a byte stream based on commands for ESC/POS printers
 */
export default class TsplLabelEncoder {
    protected _buffer: BufferValue[]
     ...
     *
     */
    protected _queue(value: BufferValue[]): void {
     ...
     *
     */
     image(element:Canvas,width:number,height:number): TsplLabelEncoder {
      ...
      const getPixel = (x: number, y: number) => image.data[((width * y) + x) * 4] > 0 ? 1 : 0;
